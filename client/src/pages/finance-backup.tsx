...
export default function Finance() {
  ...
  const updateFinanceMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({ title: "Fehler", description: error?.message || "Fehler beim Aktualisieren der Transaktion", variant: "destructive" });
    }
  });
  ...
  const deleteFinanceMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({ title: "Fehler", description: error?.message || "Fehler beim Löschen der Transaktion", variant: "destructive" });
    }
  });
  ...
  const filteredFinances = finances.filter((finance: any) => {
    const matchesSearch = 
      ...
      finance.subcategory?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const toggleFinanceStatus = (finance: any) => {
    const newStatus = finance.isActive ? 'cancelled' : 'pending';
    updateFinanceMutation.mutate({ 
      id: finance.id, 
      data: { status: newStatus, isActive: !finance.isActive }
    });
  };
